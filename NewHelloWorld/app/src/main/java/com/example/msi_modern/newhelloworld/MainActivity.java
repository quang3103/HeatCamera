...
public class MainActivity extends AppCompatActivity implements SerialInputOutputManager.Listener {
    ...
    int[][] heatData = new int[24][32];

    private Toast heatToast;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toast.makeText(this, "Graph Monitoring", Toast.LENGTH_LONG).show();
        ...
        for (int i = 0; i < 24; i++) {
            for (int j = 0; j < 32; j++) {
                ...
                arrayTextView[i][j].setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        TextView tv = (TextView) view;
                        String name = getResources().getResourceEntryName(tv.getId());

                        String[] parseName = name.split("_");
                        int u = Integer.valueOf(parseName[1]);
                        int v = Integer.valueOf(parseName[2]);
                        final Toast toast1 = Toast.makeText(getApplicationContext(), Integer.toString(heatData[u][v]), Toast.LENGTH_SHORT);
                        toast1.show();
            ...
            try {
                ...
                if (numbers.length == 768) {
                    for (int i = 0; i < 768; i++) {
                        ...
                        heatData[u][v] = Integer.valueOf(numbers[i]);
